...
struct ContentView: View {
    ...
    private let logger = Logger(subsystem: "com.titanplayer", category: "ContentView")
    ...
    private func handleDrop(providers: [NSItemProvider]) -> Bool {
        ...
        provider.loadItem(forTypeIdentifier: UTType.fileURL.identifier, options: nil) { item, _ in
            ...
            logger.debug("File dropped: \(url.path, privacy: .public)")
    ...
    private func handleFileImporterResult(_ result: Result<[URL], Error>) {
        ...
        case .success(let urls):
            ...
            logger.debug("File selected via picker: \(url.path, privacy: .public)")
            Task { @MainActor in await session.openFile(url: url) }
