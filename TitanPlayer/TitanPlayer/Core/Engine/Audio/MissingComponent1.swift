...
// MARK: - MissingComponent1

/// Audio-rendering resource governor that closes a shard-1 audit gap where
/// GPU/CPU-backed audio resources were never reclaimed under system pressure.
///
/// ## Why this exists
/// Titan Player allocates Metal-backed PCM buffers, `AVAudioEngine` node graphs,
/// and decoder scratch space that accumulate while a session is alive. The
/// original implementation left reclamation as a `TODO`, so on long sessions the
/// process drifted into memory and thermal pressure with no recovery path —
/// producing stutter, watchdog kills, and the tech-debt flagged in the audit.
///
/// ``MissingComponent1`` makes that recovery path concrete:
///
...
/// 2. On throttle/pause signals it reclaims registered audio resources within a
...
/// ### Example
/// ```swift
/// let governor = MissingComponent1(telemetry: TelemetryManager.shared)
/// governor.start()
/// let token = MissingComponent1.AudioResource(
///     label: "spatial.reverb", estimatedBytes: 4 * 1024 * 1024
...
/// ```
@MainActor
public final class MissingComponent1: Sendable {
    ...
    /// A registered audio resource the governor can reclaim under pressure.
    ///
    /// The `release` closure performs the actual teardown (freeing a Metal
    /// buffer, tearing down an `AVAudioNode` graph, etc.). It is `@Sendable`
    /// so the value type stays genuinely `Sendable` and safe to store.
    public struct AudioResource: Sendable, Identifiable {
    ...
    private let logger = Logger(subsystem: "com.titanplayer", category: "MissingComponent1")
    ...
    private var liveResources: [AudioResource] = []
    ...
    public func start() {
        ...
        logger.info("MissingComponent1 started")
    }
    ...
    public func stop() {
        ...
        monitor = nil
        cancellables.removeAll()
        logger.info("MissingComponent1 stopped")
    }

    // MARK: - Resource tracking

    /// Registers a resource for later reclamation.
    public func register(_ resource: AudioResource) {
    ...
    @discardableResult
    public func unregister(id: AudioResource.ID) -> Bool {
    ...
    ///
    /// - If the snapshot demands a pause, all resources are reclaimed.
    ...
    public func handleSnapshot(_ snapshot: SystemStateSnapshot) {
        ...
        if snapshot.shouldPauseForSystem {
            record(.compatibilityModeActivated(
                reason: "audio_pause:thermal_\(snapshot.thermal.description)_mem_\(snapshot.memory.description)",
                source: .local
            ))
            Task { try? await reclaimResources(under: snapshot) }
        } else if snapshot.shouldThrottleDecoding {
            logger.warning("Audio throttle suggested (thermal=\(snapshot.thermal.description), memory=\(snapshot.memory.description))")
            Task { try? await reclaimResources(under: snapshot) }
        } else {
            #if DEBUG
            logger.debug("Audio system nominal")
            #endif
        }
    }
    ...
    public func reclaimResources(
        ...
        record(.frameCacheEvicted(
            count: reclaimed,
            reason: "audio_pressure:thermal_\(snapshot.thermal.description)_mem_\(snapshot.memory.description)"
        ))
        logger.info("Reclaimed \(reclaimed) audio resources")
        return reclaimed
    }
    ...
    /// cancellation guard.
    ///
    /// Use this to bound third-party or driver calls (e.g. opening a Metal
    /// device, building an `AVAudioEngine` graph) so a hung acquire cannot
    /// stall the session.
    ///
    /// - Parameters:
    ...
    private func withTimeout<T: Sendable>(
        ...
        try await withThrowingTaskGroup(of: T.self) { group in
            ...
            group.addTask {
                ...
                throw MediaError(
                    code: .systemPressure,
                    message: "Audio resource operation exceeded timeout of \(duration)."
                )
            }
            ...
            do {
                guard let result = try await group.next() else {
                    throw MediaError(
                        code: .systemPressure,
                        message: "Audio resource operation produced no result."
                    )
                }
                group.cancelAll()
                return result
