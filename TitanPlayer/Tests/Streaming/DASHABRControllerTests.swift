...
final class DASHABRControllerTests: XCTestCase {
    ...
    func testStartsAtLowestQuality() throws {
        let controller = try DASHABRController(qualities: [lowQuality, midQuality, highQuality], initial: lowQuality)
        XCTAssertEqual(controller.currentQuality.id, "low")
    }

    func testStartsAtSpecifiedInitial() throws {
        let controller = try DASHABRController(qualities: [lowQuality, midQuality, highQuality], initial: midQuality)
        XCTAssertEqual(controller.currentQuality.id, "mid")
    }

    func testAvailableQualitiesSortedAscending() throws {
        let controller = try DASHABRController(qualities: [highQuality, lowQuality, midQuality], initial: lowQuality)
        XCTAssertEqual(controller.availableQualities.map(\.id), ["low", "mid", "high"])
    }

    func testSwitchUpAfterConsecutiveHighSamples() throws {
        let controller = try DASHABRController(qualities: [lowQuality, midQuality, highQuality], initial: lowQuality)
    ...
    func testSwitchDownWhenThroughputDrops() throws {
        let controller = try DASHABRController(qualities: [lowQuality, midQuality, highQuality], initial: highQuality)
    ...
    func testForceQuality() throws {
        let controller = try DASHABRController(qualities: [lowQuality, midQuality, highQuality], initial: lowQuality)
        controller.forceQuality(highQuality)
        XCTAssertEqual(controller.currentQuality.id, "high")
    }

    func testForceQualityInvalidIdIgnored() throws {
        let controller = try DASHABRController(qualities: [lowQuality, midQuality, highQuality], initial: lowQuality)
    ...
    func testCooldownPreventsRapidSwitching() throws {
        let controller = try DASHABRController(qualities: [lowQuality, midQuality, highQuality], initial: lowQuality)
